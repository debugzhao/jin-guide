...
这本质是把"Prompt 用哪个版本"从一个**编译期/进程启动期常量**，改造成一个**请求期查询**。

至少需要四个组件协同：

① 实验/灰度平台负责"这个用户应该分到哪个桶"，通常要求同一用户在会话生命周期内粘性一致（sticky bucketing）——不能同一次多轮对话里，因为缓存刷新导致上一轮和这一轮用的是不同版本的系统提示词，模型人格前后不一致；

② Prompt 内容存储（可以是 Git、也可以是数据库/远程配置中心）；

③ 一层带 TTL 或事件驱动失效的缓存，避免每个请求都去打 Prompt 存储——但纯 TTL 会有"发布后最多 N 秒才生效"的延迟，纯事件推送又要解决"推送失败导致某些实例没收到更新"的一致性问题，生产级方案通常两者都要（推送为主、TTL 兜底）；

④ 每条请求的可观测数据里必须写入"这次实际用的是哪个版本/哪个桶"，否则效果数据和版本对不上号，实验结论无法归因。

很多团队在第一次做灰度时只做了①③，却漏了④，结果是"看起来上线了 A/B 测试，但出问题时完全说不清某次故障是哪个版本触发的"。

### Q3（业务设计 + 安全）："Prompt 里写不允许做 XX"不能算安全边界，这个原则怎么落地成一套分层防御架构？

这是所有做严肃 LLM 应用的团队迟早要面对的认知升级：模型是概率系统，"提示词里禁止"只能降低违规概率，不能当成保证。

落地成架构，通常分三层，且每一层都要求"代码校验，而不是相信模型会遵守"：

**输入层**——外部内容（网页、文档、检索结果、历史摘要）进入上下文前必须标记来源并声明"不可信"，与真正的系统指令做角色隔离，防止提示注入；

**输出层**——结构化输出用 Schema 强校验而不是"只返回 JSON"这种口头约定，引用类内容做白名单 ID 校验防止编造来源，语义违规检测用独立的、fail-closed 的审查模型兜底（正则做不到的语义层面过度承诺，必须有第二层判断，且判断服务本身不可用时不能默认放行）；

**动作层**——任何有副作用的工具调用（发消息、改数据、花钱）必须有独立于 Prompt 的权限矩阵和参数校验，高风险动作加人工确认或二次审查。

这三层共同构成"模型说了不算，代码说了才算"的原则，也是 OWASP 对 Prompt Injection 防护给出的核心建议——分层防御，而不是依赖一段更强硬的系统提示词。
...
先要破除一个误区："模型无关的 Prompt 抽象层"能降低配置层面的迁移成本（模型别名、参数结构统一），但不能保证行为一致——不同模型对同一份指令的遵从度、对 Markdown/XML 结构化程度的敏感度、对"大写强调"这类技巧的反应都不同，这是模型本身的差异，不是抽象层能抹平的。

合理的架构设计是把 Prompt 拆成两层：

**业务规则层**（相对通用，讲清楚业务边界、决策流程、数据真实性要求）可以尽量模型无关；

**格式适配层**（要不要用 XML 标签、要不要给 few-shot、要不要显式要求"只输出 JSON"）承认它是模型特定的，允许同一份业务规则在不同模型下有不同的格式包装。

更关键的是流程上的硬约束：**换模型必须重新跑一遍第 Q4 提到的评测集，不能假设"提示词没变、效果就不变"**——供应商静默升级模型版本导致行为漂移是真实发生过的生产事故类型，官方文档也建议锁定模型版本并通过评测验证一致性，而不是假设升级后输出不变。

### Q6（架构设计）：多 Agent 系统里，如何设计可观测性，做到出问题能在分钟级定位是哪个 Agent、哪个 Prompt 版本、哪次工具调用导致的？

核心思路是把一次端到端请求建模成一棵 trace 树，每个 Agent 节点是一个 span，span 上必须挂三类元数据：

**身份**（prompt_name/version/hash、model_alias，用于回答"这次用的是哪份指令"）、

**性能**（延迟、token 消耗、拆分到每个节点的成本，用于回答"钱和时间花在哪了"）、

**结果归因**（成功/失败，失败要分类到具体原因——超时、非法 JSON、工具调用失败、被合规拦截，而不是笼统一个 "error"）。

生产实践上通常要两条腿走路：一条是自建的结构化审计（写自己的数据库，字段少但可靠性高、成本低，哪怕第三方观测平台挂了也不受影响）；另一条是接入专业观测平台（如 LangSmith/Datadog LLM Observability，数据更完整但依赖第三方可用性）。这两条数据流应该共享同一个 `invocation_id`/`trace_id`，出问题时才能互相印证——自建审计告诉你"哪次调用失败了"，第三方 trace 告诉你"那次调用的完整上下文和中间推理过程"。

### Q7（业务设计）：结构化输出场景（比如生成报告、推荐理由）里，function calling / 严格 JSON Schema / 纯 Prompt 要求 + 代码兜底解析，这三种方案怎么选？

这本质是"格式稳定性"和"生成质量/灵活性"之间的权衡，没有放之四海而皆准的答案，要看错误代价。

**严格 Structured Output / function calling**（模型层面保证输出符合 Schema）稳定性最高，适合"格式错误代价很高"的场景（比如要直接喂给下游系统解析的字段），代价是可能牺牲模型的自由发挥空间，某些创造性文案类任务用严格 Schema 反而会让文字变得生硬；

**纯 Prompt 要求返回 JSON + 代码校验解析**是过渡方案，胜在模型无关、迁移成本低，但必须配合 Pydantic/JSON Schema 强校验 + 有限次数重试 + **超过重试次数后必须有确定性兜底模板**——不能让"模型这次输出格式不对"直接变成用户看到的报错页面，这是很多团队上线后才踩的坑。

**选型的决策依据通常是：这个字段的下游消费者是机器还是人、格式错误的业务代价有多大、以及当前模型/网关是否支持可靠的结构化输出能力。**

### Q8（架构设计 + 业务）：如何设计一个既允许产品经理自主编辑 Prompt、又不破坏工程安全边界的系统？

**关键是把"可编辑区"和"安全外壳"在数据结构层面就物理分开**，而不是靠"大家自觉不要动不该动的部分"。

典型做法：

Prompt 定义拆成 `content`（业务话术、规则、few-shot，产品可编辑）和 `contract`（工具权限声明、输出 Schema、合规检测挂钩点，只有平台工程能改）两部分，

产品侧提供一个类似 Prompt Playground 的编辑界面（对标 LangSmith Prompt Hub 这类产品），编辑后走"保存草稿 → 自动跑评测门禁 → 通过才能提交发布 → 版本历史 + diff + 一键回滚"的流程，`contract` 部分在这个 UI 里对产品是只读的。

这样设计的好处是，"谁能改什么"变成了一个**产品能力（UI 层面的权限控制）**，而不是一份"请不要手动改这部分"的君子协定文档——后者在团队规模变大之后必然会被违反。

### Q9（业务设计）：什么时候该把巨大的 system prompt 拆成"按需动态加载的知识/流程模块"（类似 Agent Skills），什么时候不该？

决策依据不是"这是不是业界最新潮流"，而是三个具体信号：

**当前 system prompt 里，任何一次任务实际用到的内容占比是不是持续走低**（比如一个覆盖 20 个业务域的规则文档，单次任务平均只命中 1-2 个域，说明大部分 token 在陪跑、稀释模型对真正相关规则的注意力）；**业务域的数量和更新频率是不是还在持续增长**（如果未来半年要新增十几个独立场景，静态塞进同一个 system prompt 会让维护成本和注意力问题同时恶化）；**当前是否已经是多个边界清晰的窄职责 Agent**（如果本来就是"一个 Agent 只管一件事"的架构，每个 Agent 的 Prompt 天然就不大，引入动态加载反而是画蛇添足）。动态加载不是免费的最佳实践——它会新增一层"内容作为指令加载"的攻击面（第三方或用户可控的知识片段被当成指令执行）、新增版本组合的评测复杂度（要测的不再是"一份 Prompt"，而是"Prompt × 任意可能被加载的模块组合"）。只有当"膨胀带来的注意力稀释成本"明确超过"动态加载带来的架构复杂度成本"时，才值得引入。

### Q10（架构设计）：如何设计"防止 Prompt 在审批之外被偷偷改动"的机制？（供应链/内部威胁角度）

内容哈希比对（发布时登记指纹、加载时重新计算比对）只解决了"事后能不能发现内容变了"。

更完整的供应链级防护还要往前一步：**谁能把改动合并进能被生产环境读取的分支/制品，必须有强制代码属主（CODEOWNERS）卡点**，不能只靠"大家自觉走 PR"；**生产环境应该从经过 CI 构建、可信来源的制品里加载 Prompt，而不是运行时任意读取一个可能被有权限的运维直接改动的文件路径**——这一点和软件供应链安全的"不可变构建产物"思路是同一套逻辑；

**变更审计日志本身也要防篡改**（谁在什么时候改了哪个版本，这份记录如果能被同一批人悄悄改掉，审计就失去意义），成熟团队会借鉴软件供应链的签名/溯源思路（如 Sigstore/SLSA 的理念）而不只是应用层的内容哈希。内容哈希是"零成本能做的第一道线"，但面试官问到这题往往是想看你是否知道这只是纵深防御的第一层，而不是终点。
