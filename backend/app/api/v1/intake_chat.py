...
Endpoints:
  ...
  DELETE /intake/chat          — clear conversation history

取代旧版 `/profile/intent` 二分类接口：这里是一段真正的多轮流式聊天，命中建档意图
时通过 SSE `trigger_profile_capture` 事件通知前端内联渲染建档表单，见
...
def _history_key(owner_key: str) -> str:
    return f"intake:history:{owner_key}"
...
async def _load_history_from_redis(owner_key: str) -> list[dict]:
    redis_client = aioredis.from_url(settings.redis_url, decode_responses=True)
    try:
        raw = await redis_client.get(_history_key(owner_key))
...
async def _save_history_to_redis(owner_key: str, messages: list[dict]) -> None:
    redis_client = aioredis.from_url(settings.redis_url, decode_responses=True)
    try:
        await redis_client.setex(_history_key(owner_key), _REDIS_HISTORY_TTL, json.dumps(messages, ensure_ascii=False))
    except Exception:
        pass
    finally:
        await redis_client.aclose()
...
async def _persist_history_to_db(db: AsyncSession, owner_key: str, messages: list[dict]) -> None:
    """Upsert conversation history to PostgreSQL cold layer (best-effort)."""
    try:
        result = await db.execute(
            select(IntakeConversation).where(IntakeConversation.owner_key == owner_key)
        )
        ...
        if conv:
            ...
            conv = IntakeConversation(
                id=str(uuid4()), owner_key=owner_key, messages_json=messages[-_MAX_MESSAGES_STORED:]
            )
            db.add(conv)
        await db.commit()
    except Exception:
...
async def intake_chat(
    ...
    Rate limit: 30 messages per identity per day.
    """
    ...
    history = await _load_history_from_redis(owner_key)
    ...
    async def event_generator():
        ...
        async for event in stream_intake_response(history=history, user_message=message):
            ...
            if event_type == "token":
                ...
                await _save_history_to_redis(owner_key, new_history)

                from app.database import async_session_maker

                async with async_session_maker() as persist_db:
                    await _persist_history_to_db(persist_db, owner_key, new_history)
...
async def get_intake_chat_history(
    ...
    """Return the intake conversation history. Tries Redis hot layer first, falls back to PostgreSQL."""
    owner_key = _owner_key(identity)
    if not owner_key:
        return IntakeChatHistoryOut(messages=[], total=0)

    messages = await _load_history_from_redis(owner_key)
    if not messages:
        result = await db.execute(
            select(IntakeConversation).where(IntakeConversation.owner_key == owner_key)
        )
...
async def clear_intake_chat_history(
    ...
    """Clear the intake conversation history (Redis + PostgreSQL)."""
    owner_key = _owner_key(identity)
    if not owner_key:
        return
    ...
    redis_client = aioredis.from_url(settings.redis_url, decode_responses=True)
    try:
        await redis_client.delete(_history_key(owner_key))
    finally:
        await redis_client.aclose()
