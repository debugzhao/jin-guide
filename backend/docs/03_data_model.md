...
erDiagram
    ...
    auth_users ||--o{ candidate_student_profiles : "has"
    candidate_student_profiles ||--|| candidate_preferences : "has"
    candidate_student_profiles ||--o{ agent_runs : "triggers"
    candidate_student_profiles ||--o{ agent_runs_reports : "generates"
    agent_runs ||--o| agent_runs_reports : "produces"
    agent_runs_reports ||--o{ agent_runs_volunteer_checks : "includes"
    auth_users ||--o{ notify_notifications : "receives"
    rag_documents ||--o{ rag_chunks : "chunked_into"
```

> **v1.1**：`auth_sessions` 表 ORM 类名为 `AuthSession`；已删除 `human_reviews` 表及相关索引。

---

## 2. 关键表设计决策

### 2.1 enrollment_data_admission_scores 必须有 batch 字段

```sql
CREATE TABLE enrollment_data_admission_scores (
...
### 2.2 rag_chunks 表的 embedding_model 字段
...
### 2.3 agent_runs_reports.plan_json 结构设计
...
**为什么 evidence 不单独建表而是嵌入 agent_runs_reports.evidence_json**：
...
### 2.5 enrollment_data_province_thresholds 表 — 替代代码内硬编码

```sql
CREATE TABLE enrollment_data_province_thresholds (
    id              UUID PRIMARY KEY,
    province        VARCHAR NOT NULL,
    year            INTEGER NOT NULL,
...
现在通过 `enrollment_data_province_thresholds` 表配置，数据运营可以直接修改阈值，算法代码只读这张表，不需要改代码。

### 2.6 memory_intake_conversations 多会话设计

```sql
CREATE TABLE memory_intake_conversations (
...
CREATE INDEX ix_memory_intake_conversations_owner_key ON memory_intake_conversations (owner_key);
CREATE INDEX ix_memory_intake_conversations_owner_key_updated_at ON memory_intake_conversations (owner_key, updated_at, id);
```

> 消息内容不再存在这张表里——曾经有过一个 `messages_json JSONB` 列（整段对话历史存成一个数组，见下方 §2.7 "已移除"部分），P2 迁移把它拆成了独立的 `memory_conversation_messages` 表。`memory_report_conversations`（一报告一会话，报告问答 ConversationAgent 用）的结构演进与此完全同构。

最初版本 `owner_key` 有唯一约束——一个用户/匿名会话只存一条建档前聊天历史，没有会话维度，导致侧栏无法展示"新建对话 + 历史列表 + 点击恢复"（首页每次都是同一条历史）。迁移 009 去掉唯一约束，`id` 变成真正的会话/thread id，一个 owner_key 下可以有多条会话，按 `updated_at` 倒序做游标分页（参考 `agent_runs_reports` 表的分页范式）。迁移 010 补了 `deleted_at`，支持会话软删除（对齐 §4 agent_runs_reports/rag_documents 的既有约定）。
...
### 2.7 memory_conversation_messages / memory_conversation_summaries（追加式消息 + 结构化摘要，P2）

```sql
CREATE TABLE memory_conversation_messages (
    id                       VARCHAR(36) PRIMARY KEY,
    report_conversation_id   VARCHAR(36) REFERENCES memory_report_conversations(id) ON DELETE CASCADE,
    intake_conversation_id   VARCHAR(36) REFERENCES memory_intake_conversations(id) ON DELETE CASCADE,
...
CREATE TABLE memory_conversation_summaries (
    id                       VARCHAR(36) PRIMARY KEY,
    report_conversation_id   VARCHAR(36) REFERENCES memory_report_conversations(id) ON DELETE CASCADE,
    intake_conversation_id   VARCHAR(36) REFERENCES memory_intake_conversations(id) ON DELETE CASCADE,
...
**已移除**：`memory_report_conversations.messages_json` / `memory_intake_conversations.messages_json`（曾经的整段 JSONB 数组）在迁移 `016` 中被删除，历史数据已由迁移 `015` 用 `jsonb_array_elements_with_ordinality` 展开回填进 `memory_conversation_messages`（`seq` = 原数组下标）。
...
### 2.8 candidate_student_profiles / candidate_preferences 的来源与状态字段（P4 第 1 步）

迁移 `017` 给 `candidate_student_profiles`/`candidate_preferences` 各加了 7 列（详见 docs/memory-architecture.md 第六节 P4 的六段式分析，这里只记录字段本身）：
...
| `source_message_id` | 来源于某条对话消息时指向 `memory_conversation_messages.id`（`ON DELETE SET NULL`），表单提交场景恒为 `NULL` |
| `superseded_by` / `superseded_at` | 自引用，标记这条记录被哪条新记录取代、何时取代，取代链而非直接覆盖 |

**这一步只加字段，不接任何提取/确认逻辑**：`POST /profile`（唯一写入路径）在创建时把 `source_type='user_explicit'`、`status='confirmed'`、`last_confirmed_at=当前时间` 写死——一次性表单提交本身就等价于一次显式确认。历史数据由迁移 `017` 回填 `last_confirmed_at = created_at`（`candidate_preferences` 表原本连 `created_at`/`updated_at` 都没有，这次一并补上）。是否要接入"从聊天里提取候选偏好 + 用户确认"这条链路（P4 第 2 步，方案 B），留给下一次迭代评估。
...
### 2.9 投档线（admission_score）与逐分段表（rank_segment）：两张表缺一不可

这两个术语在 `enrollment_data_admission_scores`（对应 §2.1）与 `enrollment_data_rank_segments`（索引见 §3 `idx_rank_segments_lookup`）背后，是理解冲稳保推荐引擎的前提概念，必须先厘清二者的粒度和关系。

**投档线（admission_score）**：某一所学校、某一个专业组，在某一年、某一批次的最低投档分数——是"学校+专业组"粒度的数据，比如"华东理工大学(01)专业组 2025年最低560分"。

**逐分段表 / 成绩分布表（rank_segment）**：跟具体学校完全无关，是全省所有考生在某一年的"分数→累计人数"统计表——比如"2025年考622分及以上的全省有64人"。它是省级统计数据，不属于任何学校。

**为什么两个都要存，缺一不可**

核心原因是：分数本身逐年不可比，位次才可比。

- 今年考卷难，600分可能排全省第5000名；明年考卷简单，600分可能排第8000名。同一个分数在不同年份代表的竞争力完全不同。
- 但"位次"（排在全省第几名）是稳定的比较基准——今年的考生只要知道自己大概排第几名，就能直接对照往年"同样排第几名的人被这所学校这个专业组录取了"，这才是真正有参考价值的"冲稳保"判断依据。

而"投档线"（某校某专业组最低多少分）本身不带位次信息，官方公布的也只是分数。逐分段表就是唯一能把"投档线的分数"翻译成"投档线对应的位次"的查表工具——pipeline 里 `apply_admission_score_enrichment` 干的就是这件事：拿投档线的 `min_score` 去逐分段表里查那一年"考这个分数的人全省排第几"，回填成 `min_rank`。

没有逐分段表，投档线就只是一堆没法跨年比较的孤立数字；没有投档线，逐分段表就只是全省大盘统计，不知道具体学校门槛在哪。两者结合才能算出"这所学校这个专业组去年对应位次约XX名"，这才是志愿推荐引擎真正要用的数据。所以是必须的，不是可以精简掉的冗余存储。
...
CREATE INDEX idx_admission_scores_main
    ON enrollment_data_admission_scores (province, year, batch);

CREATE INDEX idx_admission_plans_major_group
    ON enrollment_data_admission_plans (province, year, batch, major_group);

-- 位次转换查询
CREATE INDEX idx_rank_segments_lookup
    ON enrollment_data_rank_segments (province, year, score);
...
MVP 阶段 rag_chunks 表数据量在 10-50 万行，HNSW 性能更优且准确度更高，选 HNSW。

---

## 4. 软删除设计

`rag_documents` 和 `agent_runs_reports` 表使用 `deleted_at` 字段实现软删除：

```sql
-- 查询时过滤软删除
SELECT * FROM agent_runs_reports
WHERE profile_id = :profile_id
  AND deleted_at IS NULL
ORDER BY created_at DESC;

-- 软删除
UPDATE agent_runs_reports SET deleted_at = NOW() WHERE id = :id;
```
...
**关键约束**：
- `Data Resolver` 在 Agent run 启动时校验 `dataset_version` 对应的所有 rag_documents 是否全为 `published` 状态
...
| `candidate_sets` | 候选集数据嵌入 `agent_runs_reports.plan_json`，数据量不大，拆表只增加 JOIN 复杂度 |
| `evidence_citations` | 证据链嵌入 `agent_runs_reports.evidence_json`，20-50 条 JSON，不需要独立表 |
