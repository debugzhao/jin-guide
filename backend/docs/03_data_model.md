...
erDiagram
    ...
    agent_runs {
        ...
        string status "queued/running/interrupted/completed/failed/timeout"
        int cost_tokens
        float cost_usd
    ...
    human_reviews {
        ...
        uuid run_id FK
        uuid reviewer_id FK
        string status "pending/in_review/need_more_info/reviewed/closed/timeout"
        jsonb checklist_json "复核清单结构"
        string conclusion "approved / rejected"
        timestamp created_at
        timestamp completed_at
        timestamp timeout_at "= created_at + 4h"
    }
    ...
    reports ||--o| human_reviews : "triggers"
...
### 2.4 human_reviews.timeout_at 字段

```sql
ALTER TABLE human_reviews
    ADD COLUMN timeout_at TIMESTAMP NOT NULL
    GENERATED ALWAYS AS (created_at + INTERVAL '4 hours') STORED;
```

定时任务扫描 `WHERE status = 'pending' AND timeout_at < NOW()`，批量将超时任务标记为 `timeout`，写 `notifications` 通知用户重新生成。

**为什么用字段而不是业务代码计算**：查询超时任务时直接 index scan，不需要在业务层做时间计算和比对。PostgreSQL generated column 保证值永远与 created_at 一致，不存在"忘记设置 timeout_at"的 Bug。
...
-- 待复核队列排序
CREATE INDEX idx_human_reviews_queue
    ON human_reviews (status, created_at ASC)
    WHERE status IN ('pending', 'in_review');  -- 部分索引，只索引活跃状态
```
