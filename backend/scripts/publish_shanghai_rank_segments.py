"""Publish the already-collected, already-valid 2025 Shanghai rank-segment
(《考生高考成绩分布表》) staging records as an immutable dataset version.

Same role as publish_jiangsu_rank_segments.py: rank-segment records are the
reference table used to enrich admission scores with min_rank (see
loaders/enrichment.py), collected as a side effect of
publish_shanghai_admission_scores.py, but need their own publish call to reach
`pipeline_published_data_records` / the `rank_segments` business table.
