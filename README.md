...
| 版本 | 日期       | 主要变更                                                                                                                                                |
| ---- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v0.8 | 2026-07-01 | 鉴权改为邮箱+密码（Resend 验证码）；移除人工复核（HITL）全流程；Agent 报告生成后直接交付 |
| v0.7 | 2026-06-28 | 完全移除家庭协同功能（含标注、冲突识别、会议议程）；删除 Section 9 家庭协同流程图；同步清理所有正文引用；章节重新编号 |
...
问津 Agent 是面向高考生和家长的 **AI 志愿决策助理**，通过考生建档、招生规则校验、冲稳保方案生成、志愿表风险体检，帮助家庭形成可解释、可追溯的志愿填报方案。
...
> **v0.8**：已移除人工复核分支（原 `L → M` 路径）。

---

## 9. 人工复核流程（已移除，v0.8）

> 2026-07-01 起不再实现 HITL。高风险报告在 UI 展示风险说明，用户自行决策。详见 `docs/backend-prd.md` Section 11。
