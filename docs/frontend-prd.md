...
| 报告详情     | `/reports/[id]`        | **M1**（hardcode）/ **M2**（真实数据） | 三套方案、证据链、风险体检、复核入口                                      |
| 志愿表体检   | `/volunteer-check`     | **M2**                                 | 手动录入志愿草稿，查看风险（文件上传为 Phase 2）                          |
| 数据源详情   | `/sources/[id]`        | **M2**                                 | 证据来源、年份、字段和可信度                                              |
| 报告历史     | `/reports`             | **M2**                                 | 用户历史报告列表                                                          |
| 人工复核     | `/reports/[id]/review` | **M3**                                 | 高风险复核清单、AI 底稿、复核结论                                         |
| 复核员工作台 | `/admin/reviews`       | **M3**                                 | 内部页面，仅 `role=reviewer/admin` 可访问；待复核队列、任务领取、提交结论 |
