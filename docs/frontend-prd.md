...
字体优先使用系统字体，保证微信内浏览器和各 Android 机型的渲染效果。
...
- 家庭协同和人工复核通过报告页内的按钮进入，不作为独立顶级入口。
...
1. 报告持有人（通常是学生或主要操作者）在报告页点击"家庭协同"。
2. 系统生成一个带 token 的分享链接，有效期 7 天。
3. 将链接通过微信发给父亲/母亲。
4. 对方打开链接，进入页面后选择自己的角色（父亲 / 母亲 / 其他）。
5. 选择角色后即可开始标注，无需注册账号。
6. 标注结果刷新后可见（当前版本不做实时推送，用户手动刷新）。

#### 页面布局

- 顶部：当前角色切换 Tab（学生 / 父亲 / 母亲）。
- 当前角色的标注列表：每张推荐卡片 + 三个标注按钮（喜欢 / 不能接受 / 有疑问）。
- 底部：冲突汇总面板（折叠/展开）。
...
| 家庭协同无标注 | 插图 + "还没有人标注，发送链接给家人" + 复制链接按钮 |
...
| Agent 超时（> 60s） | 页面内提示 + 后台继续运行 + 完成后推送通知（如有权限） |
...
| 协同 | `FamilyRoleTabs` | 角色切换 Tab |
| 协同 | `AnnotationButtons` | 喜欢 / 不能接受 / 有疑问，三按钮组 |
| 协同 | `ConflictPanel` | 冲突汇总，可折叠 |
...
- 375px 宽度下所有页面完整可用，以 390px（iPhone 14）为主要测试基准。
- 所有交互不依赖 hover 状态。
- 长报告阅读不出现横向滚动。
- 表单控件不溢出，Select 和多选使用原生控件或自定义组件。
- 底部主要操作按钮需预留 `env(safe-area-inset-bottom)` 安全区间距，避免被浏览器工具栏遮挡。

### 12.2 键盘处理

- 表单页面在软键盘弹出时，当前激活的输入框需滚动到可见区域（`scrollIntoView`）。
- 底部固定按钮在软键盘弹出时隐藏，键盘收起后恢复，避免遮挡输入框。

### 12.3 文件上传在微信内浏览器的限制

微信内浏览器对文件选择有限制，替代方案：

- 优先引导用户拍照上传志愿表截图（`accept="image/*"` + `capture`）。
- 提示用户"在微信内上传文件可能受限，建议在浏览器中打开"并给出操作路径。
- 手动录入始终作为无障碍备选方案，优先级等同于文件上传。

### 12.4 分享页

- 报告分享链接对应的页面（`/reports/[id]`）需要设置 OG 标签：
  - `og:title`：例"[学生名] 的志愿规划报告"
  - `og:description`：省份、分数、整体安全等级摘要
  - `og:image`：报告封面缩略图
