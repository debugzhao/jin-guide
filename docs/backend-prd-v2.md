...
| 429       | `login_required`       | 匿名身份超出每日对话次数上限，需登录后继续（见 §11.4） |
...
**实现**（`backend/app/agent/intake_agent.py`）：复用新增的 `intake-agent` 虚拟模型（`litellm_config.yaml`），用 OpenAI 风格 function calling 挂 5 个工具：
