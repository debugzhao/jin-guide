...
- **BFF 层**：Next.js Route Handler 承接前端请求，负责轻量鉴权、SSE 转发（含用户侧协作事件和报告问答流）、文件上传预处理。不含业务逻辑。
...
| POST   | `/api/v1/volunteer/upload`              | 上传志愿表文件，异步解析                                        |
| GET    | `/api/v1/volunteer/upload/{id}`         | 查询志愿表文件解析状态和结果                                    |
...
1. 判断 `patch` 涉及**轻量约束**还是**重大约束**：
   - 轻量约束（预算、城市偏好、排除院校/专业等不影响证据检索范围的字段）→ 只重跑 `Recommendation Agent → Risk Agent → Report Agent`，复用当前报告 checkpoint 中的 `evidence_list`/`rule_results`，不重新走 Data Resolver/Retrieval/Policy Rule，预计 5-10 秒。
   - 重大约束（省份、选科、批次变更）→ 返回 `422 requires_full_regenerate`，引导前端走完整的 `POST /api/v1/reports/generate`。
2. 轻量约束成功后创建新的 `reports` 记录：`parent_report_id` 指向原报告，`version` 在同一血缘链内递增。
3. 响应：
...
```json
{
  ...
  "diff_summary": { "candidates_before": 48, "candidates_after": 31, "changed_tiers": ["balanced"] }
}
```
...
| reports | id、profile_id、status、risk_level、risk_score、plan_json、evidence_json、dataset_version、run_id、created_at、deleted_at、**version**（同一血缘链内从 1 递增）、**parent_report_id**（指向被 refine 的原报告，首版为 null）、**run_summary_json**（用户可见的生成过程摘要，供报告页"决策过程回放"卡片使用） |
...
- `admission_probability_pct`（候选项）：精确到小数点后一位的录取概率百分比，与 `admission_safety_score`（0-100 定性安全度，驱动进度条和色彩）并存，是同一数据的两种精度呈现，不是替代关系。
...
```mermaid
flowchart TD
  SRC["数据来源"] --> FETCH["抓取/上传 documents(raw)"]
...
文件解析（OCR/PDF）为异步任务，不阻塞 API 响应；上传接口立即返回 `document_id`，前端轮询状态。
...
| 档位 | 位次差条件 | 含义 |
| --- | --- | --- |
| `high_rush`（高冲） | 学生位次 > 历史均值 5000+ | 录取概率极低，保留作为激进选项 |
...
**原则**：结构化强约束数据进 PostgreSQL；RAG 只负责解释、补充和非结构化证据；录取概率/选科/批次/体检限制必须走规则和结构化数据；证据不足时不生成强确定性结论。

**Embedding**：MVP 用 `text-embedding-3-small`（1536 维，经 LiteLLM Gateway）；Phase 2 可切换自托管 `BAAI/bge-large-zh-v1.5`（1024 维），两套模型**不能混用**，`chunks.embedding_model` 字段记录标识用于迁移过滤。
...
- `reflection_iterations >= 3` 时不再回退，**直接 best-effort 交付**（附 `compliance_issues`，用户在报告页可见具体未通过原因）。
...
未成年人数据最小化采集；敏感信息加密存储；支持用户删除档案和报告；上传文件设置过期清理策略；报告分享页有权限控制和失效机制；训练/评测/调试数据需脱敏。

### 11.3 文件上传安全

`POST /api/v1/volunteer/upload` 在 BFF 层和 API 层双重校验：文件大小 ≤10MB；MIME 白名单（PDF/Excel/JPG/PNG）；文件头魔数校验（不信任 Content-Type）；文件名过滤路径遍历字符、重命名为 UUID；独立对象存储；RAG 注入前过滤超长文本块；30 天自动清理。
...
| Reflection Agent | 3 轮未通过 | best-effort 交付（附 `compliance_issues`） |
...
- Reflection Agent 有轮次上限，实现"无需改进"早退出；3 轮未通过时正确 best-effort 交付并展示 `compliance_issues`。
...
- 文件上传接口有大小限制、MIME 白名单、文件头魔数校验；BM25 通过 `pg_bm25` 实现；`chunks` 表有 `embedding_model` 字段。
