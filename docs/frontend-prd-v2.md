...
**左侧持久导航栏**（`≥ lg` 常驻，`< lg` 收进抽屉，见 §6.1「响应式断点」）：Logo + "新建对话" + 建档前聊天（IntakeAgent）的历史会话列表（游标分页，按最近活跃倒序，点击某条恢复该会话消息并可继续对话）+ 用户菜单/登录入口（底部）。首屏默认进入全新空白对话（不预加载任何历史），历史通过侧栏显式点选恢复——`currentIntakeConversationId` 不持久化到 localStorage，刷新/重新打开 `/` 始终是新对话。
