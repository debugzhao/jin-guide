...
版本：v2.3（2026-07-09）—— 设计系统（§3）视觉基调改为 LiteLLM 式深紫黑克制风格（分析见 `docs/frontend-style.md`），风险等级色在保留红/橙/绿/蓝语义的前提下降低饱和度以适配深色背景；v2.2 变更：重新对齐 `wenjin-agent-prototype.html`，AI 对话建档页与报告工作台合并为同一组件树的两个成熟度阶段，报告不再要求生成完成后才出现或强制跳转
...
产品面向高考生和家长，这是一个高风险、高信息密度的决策场景。v2.3 起整体视觉基调参考 LiteLLM 官网（分析见 `docs/frontend-style.md`）转向**深色克制风格**：少元素、强中心、留白大、动画服务叙事而不服务装饰。设计基调应该是：

- **专业可信**：不是活泼的学生 App，也不是冰冷的政府网站，接近"可信赖的专业工具"；深色系统感传递的是技术自信，不是猎奇视觉。
- **克制大气**：参考 LiteLLM——白色负责主标题的确定性语气，淡紫强调词负责品牌记忆；模块之间留白宽裕，不堆砌卡片，一屏只讲一个重点。
- **清晰易读**：信息密度高，但层次分明，关键数字和风险项要突出；深色背景下文字对比度优先于装饰性配色。
- **风险可视**：高/中/低风险仍必须靠颜色直观区分，用户无需阅读文字就能感知危险程度；具体色值在深色主题下做了饱和度调整，见 §3.2。
...
深色主题下，背景不再是单一纯色，而是由"基色 + 装饰光晕 + 玻璃卡片"三层叠出空间感，对齐 `docs/frontend-style.md` 里"深紫黑背景 + 低饱和光晕 + 细边框玻璃卡片"的做法。

| 层级 | 色值 | 说明 |
| --- | --- | --- |
| 页面基色 | `#040128` | 深紫黑，全站统一背景，不用纯黑 `#000000` |
| 装饰光晕 | `rgba(139, 124, 246, 0.12)` 径向渐变 | 页面两侧低频模糊光源，制造空间感但不抢内容，慢速变化，不做粒子效果 |
| 玻璃卡片背景 | `rgba(255, 255, 255, 0.04)` | 卡片、面板、导航容器的背景，需配合下方边框一起使用 |
| 玻璃卡片边框 | `rgba(255, 255, 255, 0.10)` | 细白边，替代浅色主题下"阴影分层"的做法，让边框而不是阴影承担层次 |
| 弹层/Modal/Drawer 背景 | `rgba(10, 8, 38, 0.92)` + 背景模糊 | 比玻璃卡片更深，确保登录 Modal、证据链抽屉、对比抽屉在深色背景上仍有明确的最高优先级 |

#### 品牌色

| 用途   | 色值      | 说明                                               |
| ------ | --------- | -------------------------------------------------- |
| 主色   | `#A78BFA` | 淡紫，用于主按钮、链接、激活态、标题强调词；白色负责主标题的确定性语气，紫色负责品牌记忆 |
| 辅助色 | `#2DD4BF` | 青绿，用于进度条、完成状态、正向指引；亮度高于浅色主题版本，保证深色背景下的可读性 |

#### 风险等级色

这是整个产品最重要的视觉语言，必须全局统一。相比 v2.2 浅色主题版本，色值整体降低了饱和度、提高了明度以贴近深紫黑基调；红/橙/绿/蓝四种色相语义不变，但视觉冲击力比浅色主题版本弱——**这是产品决策接受的取舍**（对齐 LiteLLM 式统一克制的视觉语言），工程实现前需在真实深色背景下做一次可读性抽查，确保用户扫视时仍能第一时间识别高风险项。

| 等级          | 文字/图标色 | 背景色                        | 边框色                        | 使用场景                                                     |
| ------------- | ----------- | ----------------------------- | ----------------------------- | -------------------------------------------------------------- |
| 高风险        | `#F2A9A9`   | `rgba(242, 169, 169, 0.12)`   | `rgba(242, 169, 169, 0.32)`   | 保底不足、选科冲突、体检限制命中，需要用户重点关注并调整的项 |
| 中风险        | `#EFC48A`   | `rgba(239, 196, 138, 0.12)`   | `rgba(239, 196, 138, 0.32)`   | 梯度过密、热门专业扎堆、学费接近预算上限                     |
| 低风险 / 正常 | `#8FE0B7`   | `rgba(143, 224, 183, 0.12)`   | `rgba(143, 224, 183, 0.32)`   | 推荐理由支撑充分、规则校验通过                               |
| 提示 / 信息   | `#A9B4F5`   | `rgba(169, 180, 245, 0.12)`   | `rgba(169, 180, 245, 0.32)`   | 数据覆盖不足提示、建议补充位次                               |

#### AI 协作语义色（新增语义，非新增色值）

| 用途 | 复用色值 | 使用场景 |
| --- | --- | --- |
| AI 工具执行反馈 | 低风险色组：`#8FE0B7` 文字 + `rgba(143, 224, 183, 0.12)` 背景 | 区分"AI 直接执行了操作"与"AI 只是在回答问题" |
| Agent 并行/自检分组框 | `rgba(255, 255, 255, 0.03)` 背景 + `rgba(167, 139, 250, 0.35)` 主色边框 | 对话内生成过程卡片和报告决策回放 |

#### 中性色

深色主题下文字层级相对浅色主题反转（深底浅字）。
...
字重整体保持克制：标题大但不堆砌粗体，正文统一用次要文字色（`#9CA3C4`）弱化非关键信息，呼应 `docs/frontend-style.md` 里"没有中文海报式粗大标题堆满屏"的字体处理。

### 3.4 间距系统

基于 4px 基准：4 / 8 / 12 / 16 / 20 / 24 / 32 / 48px。整体遵循"大面积留白"原则——模块之间距离宁可大，也不把卡片塞满，这是深色主题显得贵、稳、可信的关键之一。

- 卡片内边距：16px
- 页面水平边距：16px
- 列表项间距：12px
- 章节间距：32px（相比浅色主题版本的 24px 上调一档，让每屏只突出一个重点）
...
- 卡片边框：`1px solid rgba(255, 255, 255, 0.10)`（细白边玻璃卡片，边框承担的分层作用大于阴影）
- 卡片阴影：`0 8px 24px rgba(0, 0, 0, 0.35)`（弱化阴影强度，配合边框而非替代边框）
...
AI 对话建档页是当前版本的默认首页。它不是聊天框套表单，而是一个 **Generative UI 对话流**：AI 消息可以渲染文本、结构化输入控件、风险画像、规则校验结果、档案摘要和生成过程卡片。**页面从进入起就是左右双栏结构**，与 §6.2 报告工作台共享同一布局骨架和组件——区别只是右侧面板在信息不足时是空状态，随对话推进逐步演进为"基础版报告"和"偏好更新版报告"，不是生成完成后才出现的独立目的地。

#### 页面布局（桌面端）

```
┌──────────────────────────────────────────────────────────────────────────┐
│ 顶部栏：问津 Agent / AI 对话建档 / 登录                                   │
├───────────────────────────────────┬──────────────────────────────────────┤
│ 左侧对话流                          │ 右侧实时报告面板（可折叠，默认展开）  │
│  🤖 我会用对话方式帮你完成建档...    │  状态①空：等待基础建档信息            │
│  🤖 基础建档信息卡片：省份/批次/     │                                       │
│     分数/位次/选科/体检限制          │  状态②基础版（确认必填字段后触发）：  │
│  👤 河南，本科批，612 分，32680     │   考生概况 / AI 条件点评 / 风险总览 / │
│  [确认基础信息并渲染报告]           │   三套方案 Tab / 候选卡片              │
│  🤖 生成过程卡片（默认收起摘要，     │   标签：基础版                        │
│     可展开完整并行/自检时间线）      │                                       │
│  👤 想去上海，优先计算机，学费8万内  │  状态③偏好更新版（偏好确认后触发     │
│  🤖 推荐偏好卡片：城市/专业/学费     │   /refine）：同一份报告更新候选与点评 │
│     chip，"按需生成"，非强制         │   标签：偏好更新版                    │
│  底部固定输入框：继续补充或已有志愿草稿 [发送]  [收起报告 / 悬浮"展开报告"]  │
└───────────────────────────────────┴──────────────────────────────────────┘
```
...
仅 `role=admin` 可访问，其他用户重定向 403 页面。用于开发阶段实时观察 Agent 执行状态、调试节点行为、分析性能瓶颈，**不面向普通用户开放**——用户侧的"协作可见"通过 §6.1 的对话内生成过程卡片和 §6.2 的决策回放卡片实现，Admin Debug 是面向开发者的更细粒度调试工具，两者共享同一条底层事件流，呈现层各自独立演进。本页配色**不跟随** §3.2 的深色主题改版：作为纯诊断工具，保留浅色高对比配色（见下方节点状态颜色表），优先保证开发者快速识别状态，不追求品牌视觉一致性。
