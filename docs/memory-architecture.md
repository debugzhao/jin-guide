...
| 类型                                  | 生命周期                    | 典型存储                                         | 例子                                         |
| ------------------------------------- | --------------------------- | ------------------------------------------------ | -------------------------------------------- |
| **工作记忆 Working**                  | 单次请求/单轮               | 内存/LangGraph State                             | 当前正在处理的上下文变量                     |
| **会话记忆 Conversation**             | 一个会话/thread             | Redis 热层 + Postgres 冷层，超窗口后做结构化摘要 | 你们的 P2（conversation_messages + summary） |
| **用户长期记忆 Semantic/User Memory** | 跨会话、跨账号生命周期      | 结构化字段（Postgres）或 KV，带来源/置信度/状态  | 你们的 P4（Preference/StudentProfile）       |
| **情景记忆 Episodic**                 | 关键决策/事件节点           | 向量库或时序知识图谱                             | "用户上次拒绝了某类院校推荐"                 |
| **程序性记忆 Procedural**             | 长期，跟 Agent 行为方式绑定 | Prompt/工具配置/规则库                           | 系统提示词、工具调用策略                     |
