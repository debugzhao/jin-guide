...
> MVP 阶段只做手动录入，文件上传 + OCR 解析在 Phase 2 引入。原因：OCR 解析需要异步文件处理服务，与核心技术点（Agent/RAG/HITL）无关，且占用约 2 天开发时间。MVP 入口卡片保留"上传文件"选项但展示"即将推出"提示。
...
报告页提供**探索性问答入口**，让用户在查看报告后能就具体推荐、风险和数据来源提问，由 ConversationAgent 流式回答（详见后端 PRD Section 10.9）。
...
```
错误率 0%  |  P95 延迟 42s  |  今日费用 $1.24  |  待复核 2  |  运行中 1
```
...
| -------------- | --------------------------------------------------------- |
| run_id（缩略） | 前 8 位 + 省略号                                          |
| 状态图标       | ✓ completed / ⟳ running（旋转）/ ✗ failed / ⏸ interrupted |
...
- 状态筛选：All / Running / Completed / Failed / Interrupted
- 仅显示含降级的 run（Checkbox）
- 仅显示触发人工复核的 run（Checkbox）
...
| `human_review_node`     | 人工复核 | interrupt（橙色）  | 👤   |
...
| `interrupted`           | `#7C3AED`      | `#F5F3FF` | 边框慢闪动画（等待复核）  |
...
| 条件边       | 虚线箭头     | `reflection → report`（回退循环），`reflection → [human_review or deliver]` |
| interrupt 边 | 虚线橙色     | `human_review → deliver`（resume 后）                                       |
...
| 报告问答 | `ChatMessageBubble`      | 用户/AI 消息气泡，支持行内引用标签，AI 气泡含合规修正小字        |
...
- **Debug 状态（admin 专属，非 admin 用户不初始化）**：
  ...
  - Run 列表筛选条件（`debugRunFilter: {status, degraded_only, human_review_only}`）。
