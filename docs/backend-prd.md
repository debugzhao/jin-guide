...
| 配置 | 模型 | 维度 | 适用场景 |
| ---- | ---- | ---- | ---- |
| MVP / 无 GPU | `text-embedding-3-small`（OpenAI API） | 1536 | 无 GPU 部署，快速启动 |
| 生产推荐 | `BAAI/bge-large-zh-v1.5`（自托管） | 1024 | 中文招生政策/章程理解更佳，延迟更低 |

**关键约束**：
- 两套模型**不能混用**——BGE（1024维）与 text-embedding-3-small（1536维）向量空间不兼容，混用会导致相似度完全失效。**必须选一种并全库统一**。
- `chunks.embedding_model` 字段记录每个 chunk 使用的模型标识（如 `bge-large-zh-v1.5`），模型迁移时过滤旧向量并批量重建。
- 不允许"查询用 A、文档用 B"的 fallback 混合模式。若自托管 BGE 不可用，整体降级到 OpenAI API，而非混合。
...
BM25 和向量检索可能返回同一 `chunk_id`（同一 document 的同一段落）。RRF 合并前需对结果去重：
- 以 `chunk_id` 为主键去重，保留来源 score 最高的条目。
- SQL 精确检索结果以 `source_id`（而非 chunk_id）去重，同一来源只保留一条最完整记录。
