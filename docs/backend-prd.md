...
- 当 `reflection_iterations >= 3` 时，不再回退，**直接 best-effort 交付**当前报告（附 `compliance_issues` 供前端展示）。
...
> **2026-07-01 起已完整移除**：无 `human_reviews` 表、无 `/api/v1/reviews` API、无 `human_review_node`、无 `run_agent_resume` worker 任务。高风险报告在 UI 展示风险说明，用户自行决策，不再进入复核等待流程。
