...
flowchart TD
  ...
  API --> BG["FastAPI BackgroundTasks<br/>报告生成异步执行"]
...
- **异步任务层**：报告生成通过 FastAPI `BackgroundTasks` 执行，LangGraph 在后台运行。进度事件写入 Redis，前端通过 SSE 轮询订阅。
...
| reports              | id、profile_id、status、risk_level、risk_score、plan_json、evidence_json、dataset_version、run_id            |
...
当前版本不做：`orders`、`payments`、`packages`、`coupons`、`refunds`、`invoices`、`report_versions`、`candidate_sets`、`evidence_citations`。
...
| 用途 | 模型 | 维度 | 说明 |
| ---- | ---- | ---- | ---- |
| 文档/查询向量化 | `BAAI/bge-large-zh-v1.5`（自托管）或 `text-embedding-3-small`（API fallback） | 1024 / 1536 | BGE 对中文招生政策、章程文本理解更好；API 模式无需 GPU，适合 MVP 阶段 |

文档向量化和查询向量化**必须使用同一模型**，避免 embedding 空间不一致导致相似度失效。
...
对非结构化文本，使用 **RRF（Reciprocal Rank Fusion）** 合并 BM25 全文检索和向量检索结果：
...
RRF 合并后取 top-20 进入精排。使用 `BAAI/bge-reranker-v2-m3`（自托管）或 Cohere Rerank API，精排后取 top-8 进入 Context Pack。
